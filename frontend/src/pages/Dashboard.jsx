...
const FISH_DB = {
  betta:      { tempMin:24, tempMax:28, phMin:6.5, phMax:7.5, ammMax:0.5 },
  goldfish:   { tempMin:18, tempMax:24, phMin:7.0, phMax:7.4, ammMax:0.25 },
  guppy:      { tempMin:22, tempMax:28, phMin:6.8, phMax:7.8, ammMax:0.5 },
  neon_tetra: { tempMin:20, tempMax:26, phMin:6.0, phMax:7.0, ammMax:0.25 },
  molly:      { tempMin:22, tempMax:28, phMin:7.0, phMax:8.0, ammMax:0.5 },
  platy:      { tempMin:18, tempMax:28, phMin:7.0, phMax:8.0, ammMax:0.5 },
  angelfish:  { tempMin:24, tempMax:30, phMin:6.0, phMax:7.5, ammMax:0.25 },
  oscar:      { tempMin:23, tempMax:28, phMin:6.0, phMax:8.0, ammMax:0.25 },
  discus:     { tempMin:28, tempMax:31, phMin:6.0, phMax:7.0, ammMax:0.0 },
  clownfish:  { tempMin:24, tempMax:28, phMin:8.1, phMax:8.4, ammMax:0.0 },
  blue_tang:  { tempMin:24, tempMax:26, phMin:8.1, phMax:8.4, ammMax:0.0 },
  corydoras:  { tempMin:22, tempMax:26, phMin:6.5, phMax:7.8, ammMax:0.5 },
  zebra_danio:{ tempMin:18, tempMax:24, phMin:6.5, phMax:7.0, ammMax:0.5 },
  cichlid:    { tempMin:24, tempMax:28, phMin:7.8, phMax:8.5, ammMax:0.25 },
  koi:        { tempMin:15, tempMax:25, phMin:7.0, phMax:8.6, ammMax:0.25 },
  swordtail:  { tempMin:22, tempMax:28, phMin:7.0, phMax:8.0, ammMax:0.5 },
  rainbowfish:{ tempMin:22, tempMax:28, phMin:7.0, phMax:8.0, ammMax:0.5 },
  clown_loach:{ tempMin:25, tempMax:30, phMin:6.0, phMax:7.5, ammMax:0.25 },
  tiger_barb: { tempMin:20, tempMax:26, phMin:6.0, phMax:7.0, ammMax:0.5 },
  paradise_fish:{ tempMin:16, tempMax:26, phMin:6.0, phMax:8.0, ammMax:0.5 },
  flowerhorn: { tempMin:25, tempMax:30, phMin:7.0, phMax:8.0, ammMax:0.25 },
  arowana:    { tempMin:24, tempMax:30, phMin:6.5, phMax:7.5, ammMax:0.1 },
  gourami:    { tempMin:22, tempMax:28, phMin:6.0, phMax:7.5, ammMax:0.5 },
  pleco:      { tempMin:22, tempMax:30, phMin:6.5, phMax:7.5, ammMax:0.5 },
  firemouth:  { tempMin:24, tempMax:28, phMin:6.5, phMax:8.0, ammMax:0.25 },
}
...
export default function Dashboard() {
  ...
  const fishRanges = FISH_DB[activeTank.fishId] || FISH_DB.betta
  const latest = logs[0]
  const getStatus = (log) => {
    if (!log) return 'good'
    ...
    return issues === 0 ? 'good' : issues === 1 ? 'warn' : 'critical'
  }
  ...
  const metrics = [
    { label:'Temperature', value: latest?.temperature ?? '--', unit:'°C', ok: latest ? latest.temperature>=fishRanges.tempMin && latest.temperature<=fishRanges.tempMax : true, range:`${fishRanges.tempMin}–${fishRanges.tempMax}°C`, icon:'🌡️' },
    { label:'pH Level', value: latest?.ph ?? '--', unit:'', ok: latest ? latest.ph>=fishRanges.phMin && latest.ph<=fishRanges.phMax : true, range:`${fishRanges.phMin}–${fishRanges.phMax}`, icon:'🧪' },
    ...
    <div style={{paddingTop:60,minHeight:'100vh',padding:'72px 24px 40px'}}>
      <div style={{maxWidth:1000,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
          <div>
            <h2 style={{fontSize:'1.6rem'}}>{activeTank.fishEmoji} {activeTank.name}</h2>
            <p style={{color:'var(--text2)',fontSize:'0.87rem',marginTop:4}}>{activeTank.fishName} · {activeTank.size} · {activeTank.waterType}</p>
          </div>
          <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
            {tanks.length > 1 && (
              <select value={activeTank._id} onChange={e=>setActiveTank(tanks.find(t=>t._id===e.target.value))}
                ...
                {tanks.map(t=><option key={t._id} value={t._id}>{t.fishEmoji} {t.name}</option>)}
              </select>
            )}
        ...
        <div style={{background:'linear-gradient(135deg,rgba(0,212,255,0.08),rgba(0,255,204,0.05))',border:'1px solid var(--border2)',borderRadius:'var(--radius)',padding:'16px 20px',marginBottom:20,display:'flex',gap:14,alignItems:'flex-start'}}>
          ...
          <p style={{fontSize:'0.87rem',color:'var(--text2)',lineHeight:1.7}}>
            ...
            {aiText || `Welcome! Log your first parameters to get AI-powered analysis for your ${activeTank.fishName}.`}
          </p>
        </div>
